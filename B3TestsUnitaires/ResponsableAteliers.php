
<?php

date_default_timezone_set('UTC');
require_once "Atelier.php";

class ResponsableAteliers
{

    protected $numero = 0;
    protected $nom = "Aucun Responsable";
    protected $prenom = "Aucun Responsable";

    protected $ateliers = [];


    public function __construct( $numero,  $nom,  $prenom)
    {
        $this->numero = $numero;
        $this->nom = $nom;
        $this->prenom = $prenom;
    }

    public function supResp(ResponsableAteliers $resp){
        unset($resp);
    }

    public function getNumero(): int
    {
        return $this->numero;
    }

    public function setNumero(int $numero): void
    {
        $this->numero = $numero;
    }


    public function getNom(): string
    {
        return $this->nom;
    }


    public function setNom(string $nom): void
    {
        $this->nom = $nom;
    }


    public function getPrenom(): string
    {
        return $this->prenom;
    }

    public function setPrenom(string $prenom): void
    {
        $this->prenom = $prenom;
    }


    public function getAteliers(): array
    {
        return $this->ateliers;
    }


    public function setAteliers($ateliers): void
    {
        $this->ateliers = $ateliers;
    }

    public function getNbAteliers(){
        return count($this->ateliers);
    }



    public function progAtelier(Atelier $atelier){
        $cpt = count($this->ateliers);
        if(
            $atelier->getDateProg()->format("Y-m-d") > $atelier->getDateEnreg()->format("Y-m-d")
        ){
            if(
                 date_diff($atelier->getDateProg(), $atelier->getDateEnreg())->format('%d') >= 7
                |date_diff($atelier->getDateProg(), $atelier->getDateEnreg())->format('%m') >= 1
                |date_diff($atelier->getDateProg(), $atelier->getDateEnreg())->format('%y') >= 1
            ){
                $atelier->setRespAtelier($this);
                $this->ateliers[] = $atelier;
            }
        }
        $cpt2 = count($this->ateliers);
        if(
            $cpt2 - $cpt == 1
        ){
            return true;
        }
        return false;
    }
   

    public function suppAteliers(){
        array_pop($this->ateliers);
        foreach($this->getAteliers() as $unAtelier){
            $unAtelier->setRespAtelier($this->supResp(null));
            echo $unAtelier.getNumero() . "à étais retirer";
        }
    }

    public function getAteliersAvenir(){
        $date = new DateTime("now");
        $dateFormat = $date->format('Y-m-d');

        foreach($this->ateliers as $ateliers){
            if($ateliers->getDateProg()->format('Y-m-d') > $dateFormat){
                echo $ateliers->toString() . "\n" ;
            }
        }
    }

    public function toString(): String {
        return "[RESP-ATELIERS] "
            . " Numero : " . $this->getNumero()
            . " Nom : " . $this->getNom()
            ." Prenom : " . $this->getPrenom();
    }

}
